...
internal static partial class UnroundedScaling
{
    ...
    /// </remarks>
    public static bool TryRun(double value, int requestedDigits, ref UnroundedBuffer number)
    {
#if SHORTEST_ONLY
...
#if SHORTEST_ONLY
        (digits, decimalExponent) = ShortFloat(mantissa, exponent);
#else
        if (requestedDigits == -1)
        {
            (digits, decimalExponent) = ShortFloat(mantissa, exponent);
    ...
    private static (ulong Digits, int DecimalExponent) FixedWidthFloat(ulong mantissa, int exponent, int digitCount)
    {
        ...
        if (unrounded >= ((PowersOf10[digitCount] << 2) - 2))
        {
    ...
    private static (ulong Digits, int DecimalExponent) ShortFloat(ulong mantissa, int exponent)
    {
        const int mantissaBits = 52;
        const int minimumExponent = -1022 - 63;
