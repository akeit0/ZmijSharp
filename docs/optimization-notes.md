...
The outer `double` and `float` entries mark a value irregular only when it is normal and its fraction bits are zero. Thus the inner significand is exactly `1 << 52` or `1 << 23`. [Original Żmij](https://github.com/vitaut/zmij/blob/d1682cb47e67474319ed146d3ca2c0e1a70f9429/zmij.h) detects the same case and gives it different exponent and rounding rules, but still uses the common `umul192_hi128` scaling. Here the irregular branch shifts the cached 128-bit power instead of multiplying it by that known power of two. The regular branch is unchanged. Across all normal exponents, the combined shift is 56–59 for `double` and 27–30 for `float`.

The x64 JIT private `double` conversion fell from **1,005 to 974 native bytes** with the compact cache and **631 to 610** with the full cache. In separate same-source ShortRun A/B launches, the [power-of-two benchmark](benchmark-sessions/power-of-two-short.md) measured compact `double` at **9.508 to 9.038 ns/value**, compact `float` at **9.240 to 8.681**, and full `double` at **6.649 to 6.182**. These isolate normal powers of two and are not whole-formatter or CoreLib results. Both cache profiles passed the unit suite and random/exponent-boundary verification; the local #131068 producer matched the compact profile's digits-and-scale digest over 249,884 inputs.

A pre-caller-placement decomposition ShortRun on varied inputs measured **8.932 / 11.345 / 18.955 ns/value** for compact `LongSignificand` / `Random` / `Simple`, and **6.448 / 8.516 / 15.977** for full. These are close to the earlier [pre-specialization runs](benchmark-sessions/stacked-cache-short.md); they do not show a general-workload gain.

## Caller-side scaling setup

The earlier [shared-placement trial](#shared-exponent-and-cache-placement) moved exponent, shift, and cache lookup just above the regular/irregular branch inside the private conversion method and did not improve both cache profiles. With the later power-of-two specialization in place, the same calculations were moved further out into `ToDecimal(double)` and `ToDecimal(float)`. Each outer method passes the decimal exponent, shift, and both cache words to its smaller private scale-and-round method. The private method tests its significand for the implicit bit, so a separate `regular` argument is unnecessary. The [A/B table](benchmark-sessions/shared-caller-short.md) records this placement.

The compact minimal DLL remains **12,288 B** and the full-cache one **20,480 B**. The x64 `double` entry/rounding methods changed from **298 + 974** to **576 + 486** native bytes with the compact cache, and from **298 + 610** to **399 + 486** with the full cache. Including the same adapter and digit writer, the listed call trees fell from **1,742 to 1,532 B** compact and **1,378 to 1,355 B** full. The entry still calls the private method once; cache reconstruction is inlined in the entry.

The full-cache `double` random-input A/B measured **8.563 to 6.747 ns/value** in consecutive ShortRuns. The compact result also improved in one pair. The `float` results were mixed across workloads: random input improved in the compact profile but rose slightly with the full cache; simple input moved in the opposite directions. This remains a local canonical-decomposition result, not an estimate of whole-formatter or CoreLib performance. The unit suite, two-million-pattern agreement audit, and exact-integer and exponent-boundary oracle checks passed.

## Nonzero last digit

The `double` entry now returns the canonical result directly when the private conversion supplies a nonzero last digit. Its trailing-zero normalization loop is still used when that digit is zero or absent. In the [ShortRun A/B](benchmark-sessions/nonzero-last-digit-short.md), compact decomposition changed from **8.591 / 11.046 / 18.792** to **8.229 / 10.450 / 18.551 ns/value** for long-significand / random / simple corpora. Full-cache results changed from **6.060 / 6.764 / 15.514** to **5.605 / 6.581 / 15.466** in separate launches. A second source form that also absorbed a zero extra digit into the exponent was tried; its compact long-significand and full random results were worse, so the first form was retained.

The minimal DLL lengths remain **12,288 B compact** and **20,480 B full**. The x64 entry grows by 30 and 28 native bytes respectively, but its nonzero-digit path branches around the `% 10` sequence. Both cache profiles passed the unit suite; the compact profile passed the two-million-pattern and exponent-boundary comparison and matched the pinned #131068 local port's digits-and-scale digest.

The later source cleanup gives `ZmijDecimal` a raw constructor and an explicit `CreateNormalized` factory for paths that still need trailing-zero removal. The fast `double` path now uses the raw constructor. Address-normalized x64 JIT listings for `ToDecimal(double)` are instruction-identical to the pre-cleanup version in both cache profiles, and both minimal DLL lengths are unchanged. The [session record](benchmark-sessions/nonzero-last-digit-short.md) includes the compact rerun and checks.
