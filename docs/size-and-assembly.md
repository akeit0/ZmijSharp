...
This record compares the **finite, nonzero `double` shortest-digit producer** from Zmij with the older local unrounded-scaling snapshot. It does not compare standalone formatters or estimate an incremental `System.Private.CoreLib` change. Source arrays, PE DLL length, and JIT instruction bytes are separate measurements.
...
| Current #131068 and local snapshot cached powers | 696 × two `ulong` | 11,136 |

Grisu3 is counted from [`Number.Grisu3.cs` at `dotnet/runtime` `f0df4333`](https://github.com/dotnet/runtime/blob/f0df4333553c63a6bdba26228ab94b99e4a1c8d1/src/libraries/System.Private.CoreLib/src/System/Number.Grisu3.cs) (2026-09-25). Its producer is generic and uses one set of arrays, so the 1,084-byte count is not multiplied by the number of floating-point types. The current PR's [`Number.Pow10Table.cs` at `56ff8516`](https://github.com/dotnet/runtime/blob/56ff851680b3c64a9ecaf543225b9cc948fe0262/src/libraries/System.Private.CoreLib/src/System/Number.Pow10Table.cs) contains the same 696 high/low pairs as the [older local snapshot](../src/UnroundedScaling.Comparison/SOURCE.md); the source values were compared directly.

The PR's [generic producer](https://github.com/dotnet/runtime/blob/56ff851680b3c64a9ecaf543225b9cc948fe0262/src/libraries/System.Private.CoreLib/src/System/Number.UnroundedScaling.cs) shares that table across `double`, `float`, `Half`, and `BFloat16`. **The 16-bit types add no separate cached-power data.** The [PR description](https://github.com/dotnet/runtime/pull/131068) attributes their additional size to native generic specializations. These counts exclude alignment, metadata, native code, and other formatter data. The local snapshot's bounded-precision helper also defines a 160-byte small-power table; that helper is excluded from the shortest-only DLL below.
...
| `ShortestCore=Unrounded` | **18,944 B** |

Both assemblies are built by the same project settings and differ by **7,168 B** as PE files. The comparison is meaningful for these two *isolated source snapshots* and their identical public adapter. It is not a CoreLib image delta or a result for the current #131068 implementation. PE section rounding, metadata, and IL are included in the file lengths.
...
| Method role | Zmij compact | Local unrounded snapshot |
|---|---:|---:|
| Public adapter | 171 B | 246 B |
| Decode input | `ToDecimal(double)` 520 B | `ExtractFractionAndBiasedExponent` 64 B |
| Shortest conversion | `ToDecimal(ulong, int, bool)` 1,097 B | `ShortFloat` 659 B |
| Digit writing | `TryGetSignificantDigits(ZmijDecimal, …)` 299 B | `StoreDigits` 129 B + `CountDigits` 44 B + `UInt64ToDecChars` 131 B |
| **Listed call-tree total** | **2,087 B** | **1,273 B** |

The compact Zmij path spends instructions reconstructing cached powers: it indexes 28 minors and 23 anchors, multiplies the words, normalizes the result, and applies one correction bit. The unrounded snapshot reads adjacent table words. Native code is larger for Zmij in this JIT run while its counted data and DLL are smaller. These method totals are observations from one architecture and JIT; they cannot be added to the DLL lengths or used as a ReadyToRun estimate.
