...
- A current-source, same-session standalone `double` `TryFormat` workload run shows Zmij near parity on JSON-like values (30.22 vs 31.02 ns) and faster on uniform raw-bit values (45.38 vs 90.41 ns). It ran in one ShortRun; the comparison port of #131068 is pinned to an older snapshot and does not measure current CoreLib. See [methods and full tables](benchmark-results.md).
...
- A source-linked minimal project now measures the `double` shortest producers behind the same public adapter: 11,776 B for compact Zmij and 18,944 B for the older unrounded-scaling snapshot. Both builds round-tripped and produced identical digits and scales for 249,884 deterministic finite nonzero values. See the [size and assembly record](size-and-assembly.md) for source selection and limitations.
