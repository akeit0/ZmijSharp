...
- The [three-producer default-job benchmark](benchmark-sessions/shortest-decomposition-default.md) builds compact, full, and the pinned #131068 local port together. Its setup checked equal canonical tuples on each corpus. In one BenchmarkDotNet invocation, compact/full/#131068 measured **8.049 / 5.613 / 6.817 ns** on varied long significands and **10.055 / 6.164 / 8.613 ns** on random raw bits. The [earlier ShortRun](benchmark-sessions/same-run-cache-profiles-short.md) had the same ordering. BenchmarkDotNet launches each case in its own process; this remains a standalone `double` decomposition comparison.
