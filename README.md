...
- In the [current decomposition comparison](docs/benchmark-sessions/nonzero-last-digit-short.md), the local #131068 port was faster than compact Zmij on the three measured corpora; full-cache Zmij was faster on two, with a larger table. Pointer and span digit buffers were near parity in [component benchmarks](docs/component-benchmarks.md). The nonzero-last-digit fast path improved the local `double` decomposition workloads in A/B runs, with no minimal DLL growth.
