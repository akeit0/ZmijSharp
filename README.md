...
- In the [current decomposition comparison](docs/benchmark-sessions/current-cache-comparison-short.md), the local #131068 port was faster than compact Zmij on varied inputs; full-cache Zmij was faster on two of the three measured corpora in one launch, with a larger table. Pointer and span digit buffers were near parity in [component benchmarks](docs/component-benchmarks.md). Moving scaling setup into Zmij's caller reduced its native call tree and improved some [local A/B workloads](docs/benchmark-sessions/shared-caller-short.md).
